    ...
    async findAmenities(maxDistance = 100) {
        ...
        for (const [type, query] of Object.entries(queries)) {
            try {
                ...
                // Try direct API first, fall back to CORS proxy if needed
                let apiUrl = 'https://overpass-api.de/api/interpreter';
                
                // Check if we're in development and use CORS proxy
                if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
                    console.log(`Using CORS proxy for ${type} (development mode)`);
                ...
                if (!data || !data.elements) {
                    console.warn(`No elements returned for ${type}`);
                    results[type] = [];
                    continue;
                }
                ...
                console.log(`Falling back to empty results for ${type}`);
                results[type] = [];
            }
        }
...
export default GPXAmenityFinder;